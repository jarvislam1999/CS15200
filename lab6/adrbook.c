...
bst *do_read(char *infile)
{
  ...
  while (!feof(f)) {
    ...
    if (address_book) {
      ...
      if (i==0) /* c wasn't inserted in tree; free it */
	vcard_free(c);
    }
    else
...
/* do_substring: execute the SUBSTRING command: 
 * display all vcards whose cnet contains substring
 */
void do_substring(char *sub, bst *address_book)
{
  unsigned int z = bst_substring(stdout,address_book,sub);
  printf("\n(found %u CNETs including \"%s\")\n\n",z,sub);
}
...
void do_cmd(cmd *c, bst **address_book, int *quit)
{
  ...
  case SUBSTRING: do_substring(c->arg, *address_book); break;
