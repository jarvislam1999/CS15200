...
/* cmd_substring: make a new heap-allocated SUBSTRING command */
/* note: copy s (deep copy) */
cmd *cmd_substring(char *sub)
{
  unsigned int len = strlen(sub);
  if (len==0) {
    shell_error("no string given with substring command");
    return NULL;
  }
  cmd *c = cmd_new(SUBSTRING);
  c->arg = strdup(sub);
  return c;
}
...
cmd *cmd_from_string(char *s)
{
  ...
  if (strlen(cmdbuf)==1) {
    ...
    case 'b' : return cmd_substring(argbuf);
